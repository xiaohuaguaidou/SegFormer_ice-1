...
if __name__ == "__main__":
    ...
    if local_rank == 0:
        ...
        macs, params = get_model_complexity_info(
            ...
            as_strings=True,
            print_per_layer_stat=True,  # 改为True可以看到每层的统计
            verbose=False
        )
        print(f"\n=== 模型复杂度分析 ===")
        print(f"计算量 (FLOPs): {macs}")
        print(f"参数量: {params}")
    ...
    if True:
        ...
        for epoch in range(Init_Epoch, UnFreeze_Epoch):
            ...
            fit_one_epoch(model_train, model, loss_history, eval_callback, optimizer, epoch, epoch_step, epoch_step_val,
                          gen, gen_val, UnFreeze_Epoch, Cuda, dice_loss, focal_loss, cls_weights, num_classes, fp16, scaler, save_period, save_dir=log_dir,
